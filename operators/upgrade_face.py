...
def find_face_bone(obj):
    ...
    if pbone and get_rigify_type(pbone) == 'game.faces.super_face':
        return pbone.name
...
def process_all(process, name_map):
    ...
    process('nose', rig='game.skin.stretchy_chain', connect_ends='next', priority=1)
    process('nose.001')
    process('nose.002', parent='nose_master',
            rig='game.skin.stretchy_chain', connect_ends=True, priority=1)
    process('nose.003')
    process('nose.004', parent='face', rig='game.skin.basic_chain', connect_ends='prev')

    process('lip.T.L', parent='jaw_master', layer=1, rig='game.skin.stretchy_chain', sharpen=(0, 90),
            falloff=(0.7, 1, 0.1), spherical=(True, False, True), scale=True)
    process('lip.T.L.001')

    process('lip.B.L', parent='jaw_master', layer=1, rig='game.skin.stretchy_chain', sharpen=(0, 90),
            falloff=(0.7, 1, 0.1), spherical=(True, False, True), scale=True)
    process('lip.B.L.001')

    process('jaw', parent='jaw_master', layer=1, rig='game.skin.basic_chain', connect_ends='next')
    process('chin', parent='jaw_master', rig='game.skin.stretchy_chain', connect_ends='prev')
    process('chin.001')
    ...
    process('lip.B.R', parent='jaw_master', layer=1, rig='game.skin.stretchy_chain', sharpen=(0, 90),
            falloff=(0.7, 1, 0.1), spherical=(True, False, True), scale=True)
    process('lip.B.R.001')

    process('brow.B.L', rig='game.skin.stretchy_chain', middle=2, connect_ends='next')
    process('brow.B.L.001')
    process('brow.B.L.002')
    process('brow.B.L.003')

    # ,connect_ends=True,sharpen=(120,120))
    process('lid.T.L', parent='eye.L', sec_layer=1, rig='game.skin.stretchy_chain', middle=2,
            spherical=(False, True, False))
    process('lid.T.L.001')
    process('lid.T.L.002')
    process('lid.T.L.003')
    # ,connect_ends=True,sharpen=(120,120))
    process('lid.B.L', parent='eye.L', sec_layer=1, rig='game.skin.stretchy_chain', middle=2)
    process('lid.B.L.001')
    process('lid.B.L.002')
    process('lid.B.L.003')

    process('brow.B.R', rig='game.skin.stretchy_chain', middle=2, connect_ends='next')
    process('brow.B.R.001')
    process('brow.B.R.002')
    process('brow.B.R.003')

    # ,connect_ends=True,sharpen=(120,120))
    process('lid.T.R', parent='eye.R', sec_layer=1, rig='game.skin.stretchy_chain', middle=2,
            spherical=(False, True, False))
    process('lid.T.R.001')
    process('lid.T.R.002')
    process('lid.T.R.003')
    # ,connect_ends=True,sharpen=(120,120))
    process('lid.B.R', parent='eye.R', sec_layer=1, rig='game.skin.stretchy_chain', middle=2)
    process('lid.B.R.001')
    process('lid.B.R.002')
    process('lid.B.R.003')

    process('forehead.L', parent='face', rig='game.skin.basic_chain')
    process('forehead.L.001', parent='face', rig='game.skin.basic_chain')
    process('forehead.L.002', parent='face', rig='game.skin.basic_chain')

    process('temple.L', parent='face', rig='game.skin.basic_chain', connect_ends=False, priority=1)
    process('jaw.L', parent='jaw_master', rig='game.skin.basic_chain', connect_ends='prev')
    process('jaw.L.001')
    process('chin.L')
    process('cheek.B.L', parent='face', layer=1, rig='game.skin.stretchy_chain', connect_ends='next')
    process('cheek.B.L.001')
    process('brow.T.L', parent='face', rig='game.skin.basic_chain', connect_ends=True)
    process('brow.T.L.001', parent='face', layer=1, rig='game.skin.stretchy_chain', connect_ends=True)
    process('brow.T.L.002')
    process('brow.T.L.003', parent='face', rig='game.skin.basic_chain', connect_ends='prev')

    process('forehead.R', parent='face', rig='game.skin.basic_chain')
    process('forehead.R.001', parent='face', rig='game.skin.basic_chain')
    process('forehead.R.002', parent='face', rig='game.skin.basic_chain')

    process('temple.R', parent='face', rig='game.skin.basic_chain', connect_ends=False, priority=1)
    process('jaw.R', parent='jaw_master', rig='game.skin.basic_chain', connect_ends='prev')
    process('jaw.R.001')
    process('chin.R')
    process('cheek.B.R', parent='face', layer=1, rig='game.skin.stretchy_chain', connect_ends='next')
    process('cheek.B.R.001')
    process('brow.T.R', parent='face', rig='game.skin.basic_chain', connect_ends=True)
    process('brow.T.R.001', parent='face', layer=1, rig='game.skin.stretchy_chain', connect_ends=True)
    process('brow.T.R.002')
    process('brow.T.R.003', parent='face', rig='game.skin.basic_chain', connect_ends='prev')

    process('eye.L', layer=0, rig='game.face.skin_eye')
    process('eye.R', layer=0, rig='game.face.skin_eye')

    process('cheek.T.L', rig='game.skin.basic_chain')
    process('cheek.T.L.001')

    process('nose.L', parent='brow.B.L.004', connect=True)
    process('nose.L.001', parent='nose_master', rig='game.skin.basic_chain', layer=1)

    process('cheek.T.R', rig='game.skin.basic_chain')
    process('cheek.T.R.001')

    process('nose.R', parent='brow.B.R.004', connect=True)
    process('nose.R.001', parent='nose_master', rig='game.skin.basic_chain', layer=1)

    process('teeth.T', layer=0, rig='game.basic.super_copy', params={'super_copy_widget_type': 'teeth'})
    process('teeth.B', layer=0, parent='jaw_master', rig='game.basic.super_copy',
            params={'super_copy_widget_type': 'teeth'})

    process('tongue', pri_layer=0, parent='jaw_master', rig='game.face.basic_tongue')
    process('tongue.001')
    process('tongue.002')

    # New bones
    process('jaw_master', layer=0, parent='face', rig='game.face.skin_jaw',
            params={'jaw_mouth_influence': 1.0})
    process('nose_master', layer=0, parent='face', rig='game.basic.super_copy',
            params={'super_copy_widget_type': 'diamond', 'make_deform': False})

    process('brow.B.L.004', parent='face', rig='game.skin.stretchy_chain',
            connect_ends='prev', falloff=(-10, 1, 0))
    process('brow.B.R.004', parent='face', rig='game.skin.stretchy_chain',
            connect_ends='prev', falloff=(-10, 1, 0))

    process('brow_glue.B.L.002', parent='face', rig='game.skin.glue', glue_copy=0.25, glue_reparent=True)
    process('brow_glue.B.R.002', parent='face', rig='game.skin.glue', glue_copy=0.25, glue_reparent=True)

    process('lid_glue.B.L.002', parent='face', rig='game.skin.glue', glue_copy=0.1)
    process('lid_glue.B.R.002', parent='face', rig='game.skin.glue', glue_copy=0.1)

    process('cheek_glue.T.L.001', parent='face',
            rig='game.skin.glue', glue_copy=0.5, glue_reparent=True)
    process('cheek_glue.T.R.001', parent='face',
            rig='game.skin.glue', glue_copy=0.5, glue_reparent=True)

    process('nose_glue.L.001', parent='face', rig='game.skin.glue', glue_copy=0.2, glue_reparent=True)
    process('nose_glue.R.001', parent='face', rig='game.skin.glue', glue_copy=0.2, glue_reparent=True)

    process('nose_glue.004', parent='face', rig='game.skin.glue', glue_copy=0.2, glue_reparent=True)

    if 'nose_end_glue.004' in name_map:
        process('nose_end_glue.004', parent='face', rig='game.skin.glue', glue_copy=0.5, glue_reparent=True)

    process('chin_end_glue.001', parent='jaw_master', rig='game.skin.glue', glue_copy=0.5, glue_reparent=True)
...
def set_rig(
    ...
    if rig is not None:
        ...
        if rig in ('game.skin.basic_chain', 'game.skin.stretchy_chain', 'game.skin.anchor'):
        ...
        if scale:
            if rig == 'game.skin.stretchy_chain':
...
class POSE_OT_gamerig_upgrade_game_face(bpy.types.Operator):
    ...
    bl_idname = "pose.gamerig_upgrade_game_face"
    bl_label = "Upgrade Game Face Rig"
...
classes = (
    POSE_OT_gamerig_upgrade_game_face,
)
